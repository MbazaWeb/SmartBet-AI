    ...
    <main className="route-shell-enter mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      ...
      <section className="mt-8 grid gap-6 xl:grid-cols-[1fr_340px]">
        <div className="space-y-6">
          <ResultsSummary loading={dashboard.loading} resultsCards={dashboard.resultsCards} />
          <CalibrationPanel loading={dashboard.loading} calibrationBands={dashboard.calibrationBands} />
          <PlayedResultsPanel loading={dashboard.loading} playedResultInsights={dashboard.playedResultInsights} outcomeLabels={dashboard.outcomeLabels} />
        </div>

        <aside className="space-y-5">
          <LivePollsPanel loading={dashboard.loading} livePolls={dashboard.livePolls} handleVote={dashboard.handleVote} />
          <PredictionNotesPanel loading={dashboard.loading} researchDigest={dashboard.researchDigest} teamIntel={dashboard.teamIntel} />
        </aside>
      </section>
    </main>
  )
